    ...
    if contours:
        ...
        if cv2.contourArea(c) > 1000:
            ...
            if len(approx) == 4:
                ...
                cv2.putText(frame, "Left Edge", ((int(pt_left_top[0]+pt_left_bottom[0])//2, int(pt_left_top[1]+pt_left_bottom[1])//2-10)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)
                cv2.line(frame, tuple(pt_right_top.astype(int)), tuple(pt_right_bottom.astype(int)), (0, 0, 255), 3)
                cv2.putText(frame, "Right Edge", ((int(pt_right_top[0]+pt_right_bottom[0])//2, int(pt_right_top[1]+pt_right_bottom[1])//2-10)),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

                # 計算中心線兩端點
                center_top = ((pt_left_top + pt_right_top) / 2).astype(int)
                center_bottom = ((pt_left_bottom + pt_right_bottom) / 2).astype(int)
                cv2.line(frame, tuple(center_top), tuple(center_bottom), (0, 255, 255), 3)
                cv2.putText(frame, "Center Line", ((center_top[0]+center_bottom[0])//2, (center_top[1]+center_bottom[1])//2-10),
