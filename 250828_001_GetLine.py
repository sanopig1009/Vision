    ...
    if contours:
        ...
        if cv2.contourArea(c) > 1000:
            ...
            if len(approx) == 4:
                ...
                # 畫出左邊線（左上到左下）
                pt_left_top = tuple(rect[0].astype(int))
                pt_left_bottom = tuple(rect[3].astype(int))
                cv2.line(frame, pt_left_top, pt_left_bottom, (255, 0, 255), 3)
                cv2.putText(frame, "Left Edge", ((pt_left_top[0]+pt_left_bottom[0])//2, (pt_left_top[1]+pt_left_bottom[1])//2-10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 255), 2)

                # 畫出右邊線（右上到右下）
                pt_right_top = tuple(rect[1].astype(int))
                pt_right_bottom = tuple(rect[2].astype(int))
                cv2.line(frame, pt_right_top, pt_right_bottom, (0, 0, 255), 3)
                cv2.putText(frame, "Right Edge", ((pt_right_top[0]+pt_right_bottom[0])//2, (pt_right_top[1]+pt_right_bottom[1])//2-10),
